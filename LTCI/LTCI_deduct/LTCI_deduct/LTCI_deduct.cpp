					...
					for(deduct=0;deduct<t;deduct++)
						...
						(1/(1+r))*(   q[t+1][j*5+0]*lcp->insurance2[0][deduct][tempIndex]
								...
								+q[t+1][j*5+3]*lcp->insurance2[3][deduct][tempIndex]);
									...
									lcp->insurance[j][deduct][i]= lcp->insurance[j][deduct][i] + 
					(1/(1+r))*(   q[t+1][j*5+0]*lcp->insurance2[0][deduct+1][tempIndex]
								+q[t+1][j*5+1]*lcp->insurance2[1][deduct+1][tempIndex]
								+q[t+1][j*5+2]*lcp->insurance2[2][deduct+1][tempIndex]
								+q[t+1][j*5+3]*lcp->insurance2[3][deduct+1][tempIndex]);
...
void Parallel_LTCI::simulate(int wealthpercentile){
	...
	int healindex;
	int (*Astream)[TN][4][MAXGRID];
	int sum_med_joint_NI,sum_med_joint;
	...
	double surplus;
	double r,wextra;
	double sum_Medicaid,sum_insurance,sum_medcost,sum_MUedicaid,sum_medicare;
	double sum_testins,sum_testmed,sum_utility;
	double sum_OOP_NI, sum_OOP;
	...
	double *wdis;
		...
		typedef struct{
			...
			int med_joint_NI[NSIMUL]; // medicaid participation number of people for NO insurance
			int med_joint[NSIMUL];	// medicaid participation number of people for insurance
			...
			double OOP_NI[NSIMUL]; // med expense for out of pocket NO insurance
			double OOP[NSIMUL]; // med expense for out of pocket insurance
			double Medicare[NSIMUL]; // med expense for Meidcare   
		...
		sum_med_joint=0;sum_med_joint_NI=0; sum_OOP_NI=0;sum_OOP=0;sum_medicare=0;sum_utility=0;
			 ...
			 if(healthstate[i][t]!=4){
				 ...
				 if(para.Mcaid==1 &&  constraint < (Cbar2+Wbcar) && constraint-wdis[s->curWW[i]]/(1+r) < Cbar2 && healthstate[i][t]==1)
				 {
					 ...
					 if(ii==0)
					 {
						 ...
						 if(s->tempIndex[i] > deductgrid-2)s->insurance[i] =s->insurance[i] + (1/rfactor[t])*LTCI[t][healthstate[i][t]];
					 }
				 ...
				 }else if(para.Mcaid==1 &&  constraint < (Cbar+Wbcar) &&  constraint-wdis[s->curWW[i]]/(1+r)< Cbar && healthstate[i][t] > 1 )
				 {
					 ...
					 s->nextWW[i]=Astream[s->tempIndex[i]][t][healthstate[i][t]][s->curWW[i]];
					  ...
					  {
						  s->med_joint_NI[i]=1;
						  s->MUedicaid[i]=s->MUedicaid[i]+ (1 / rfactor[t])*(M[t][healindex]-max((double)0,(double)(wextra+(A+r*wdis[s->curWW[i]]/(1+r) -Cbar))));
					  }
					  else{
						 s->med_joint[i]=1;
						 s->Medicaid[i]=s->Medicaid[i]+ (1/ rfactor[t])*((M[t][healindex]-LTCI[t][healindex]*(s->tempIndex[i] > deductgrid-2)-LTCI[t][0]*(s->tempIndex[i] <= deductgrid-2 ))-max((double)0,(double)(wextra+(A+r*wdis[s->curWW[i]]/(1+r) -Cbar))));
						 if(s->tempIndex[i] > deductgrid-2)s->insurance[i] =s->insurance[i] + (1/rfactor[t])*LTCI[t][healthstate[i][t]];
 ...
 for(i=0;i<NSIMUL;i++){
	 ...
	 sum_med_joint_NI=sum_med_joint_NI+s->med_joint_NI[i];
	 sum_med_joint=sum_med_joint+s->med_joint[i];
	 sum_OOP=sum_OOP+s->OOP[i];
	 sum_OOP_NI=sum_OOP_NI+s->OOP_NI[i];
	 sum_medicare=sum_medicare+s->Medicare[i];
...
//  sum_med_joint_NI=sum_med_joint_NI/nsimul;
//  sum_med_joint=sum_med_joint/nsimul;
 sum_OOP=sum_OOP/nsimul;
 sum_OOP_NI=sum_OOP_NI/nsimul;
 sum_medicare=sum_medicare/nsimul;

 Digram.S_Medicaid=sum_Medicaid;
 Digram.S_Insurance=sum_insurance;
 Digram.Medcost=sum_medcost;
 Digram.S_Medicaid_NI=sum_MUedicaid;
 Digram.S_med_joint=(double)sum_med_joint/nsimul;
 Digram.S_med_joint_NI=(double)sum_med_joint_NI/nsimul;
 Digram.S_medicare=sum_medicare;
 Digram.S_OOP=sum_OOP;
 Digram.S_OOP_NI=sum_OOP_NI;
