...
static double Preimum[10][2]={   
	0.153195441355678,   0.072662610073732,
	0.149253723001179,   0.070178747039844,
	0.142876163832978,   0.067078037165284,
	0.137348989176930,   0.062682442701983,
	0.132906038097650,   0.060595252004067,
	0.131275627740406,   0.058837033175706,
	0.124146008856344,   0.055679551849277,
	0.123293671499981,   0.053498875537092,
	0.119848915524493,   0.052723533470760,
	0.115932229699623,   0.050756558169127
};
		...
		*/
		...
		for(j=0;j<4;j++)
		{
			...
			for(deduct=0;deduct<deductgrid;deduct++)	
				...
				{
					if(j!=3)
						constraint=A+LTCIMM[t][j]+wdis[i]-M[t][j];// 这个写法满足要求
					else
						constraint=A+wdis[i]+(LTCIMM[t][j]-M[t][j])*(deduct+(j>0) > deductgrid-2)+LTCIMM[t][0]*(deduct+(j>0) <= deductgrid-2);
					...
					if(para.Mcaid==1 &&  constraint < (Cbar2+Wbcar) && constraint-wdis[i]/(1+r) < Cbar2 && j==1)
					{
						...
						lcp->Medicaid[j][deduct][i]= (M[t][j]-LTCIMM[t][j] -max((double)0,(double)(wextra+(A+r*wdis[i]/(1+r) -Cbar2))));
						lcp->insurance[j][deduct][i] = LTCIMM[t][j];
					...
					}else if(para.Mcaid==1 &&  constraint < (Cbar+Wbcar) &&  constraint-wdis[i]/(1+r)< Cbar && j>1 )
					{
						...
						if(j==2){
							lcp->insurance[j][deduct][i] = LTCIMM[t][j];
							lcp->Medicaid[j][deduct][i]= (M[t][j]-LTCIMM[t][j])-max((double)0,(double)(wextra+(A+r*wdis[i]/(1+r) -Cbar)));
						}else
						{
						...
						{
							lcp->c[deduct][k]=wdis[i]-wdis[k]/(1+r) +A +(LTCIMM[t][j]-M[t][j])*(1-(j==3)*(deduct+(j>0)<=deductgrid-2))+(LTCIMM[t][0]*(j==3)*(deduct+(j>0)<=deductgrid-2));
							if(lcp->c[deduct][k]>0 && wdis[i]+A+(LTCIMM[t][j]-M[t][j])*(1-(j==3)*(deduct+(j>0)<=deductgrid-2))+(LTCIMM[t][0]*(j==3)*(deduct+(j>0)<=deductgrid-2))-lcp->c[deduct][k]>=0)lcp->zeroind[deduct][k]=0;else{lcp->c[deduct][k]=0.0001;lcp->zeroind[deduct][k]=1;}
						}

						lcp->Medicaid[j][deduct][i]=0;
						if (j>=1 && j!=3 ) lcp->insurance[j][deduct][i]=LTCIMM[t][j];
						if(j==3)lcp->insurance[j][deduct][i]=LTCIMM[t][j]*(deduct+(j>0)>deductgrid-2);
				...
				for(deduct=0;deduct<deductgrid;deduct++)
					...
					}else if(para.Mcaid==1 &&  constraint < (Cbar+Wbcar) &&  constraint-wdis[i]/(1+r)< Cbar && j>1 )
					{
						...
						for(k=0;k<CalcStruct.wrow;k++)
							...
							else if(j==3 && deduct<deductgrid-1)
								lcp->VV[j][deduct][k]=lcp->VV[j][deduct][k]+
								(1/(1+para.rho))*q[t+1][j*5  ]*lcp->V[0][0][k]
							+(1/(1+para.rho))*q[t+1][j*5+1]*lcp->V[1][0][k]
							+(1/(1+para.rho))*q[t+1][j*5+2]*lcp->V[2][0][k]
			...
			for(t=deductgrid-2;t>=0;t--)
			{
				...
				for (j=0;j<4;j++)
					...
					if(j!=3)
						...
						constraint=A+wdis[i]+LTCIMM[t][0];
					...
					}else if(para.Mcaid==1 &&  constraint < (Cbar+Wbcar) &&  constraint-wdis[i]/(1+r)< Cbar && j>1 )
					{
						...
						{
							lcp->Medicaid[j][deduct][i]= -LTCIMM[t][0] -max((double)0,(double)(wextra+(A+r*wdis[i]/(1+r) -Cbar)));
						}
						...
						for(k=0;k<CalcStruct.wrow;k++)
						{
							lcp->c[deduct][k]=wdis[i]-wdis[k]/(1+r) +A +(LTCIMM[t][j]-M[t][j])*(1-(j==3))+(LTCIMM[t][0]*(j==3));
							if(lcp->c[deduct][k]>0 && wdis[i]+A+(LTCIMM[t][j]-M[t][j])*(1-(j==3))+(LTCIMM[t][0]*(j==3))-lcp->c[deduct][k]>=0)lcp->zeroind[deduct][k]=0;else{lcp->c[deduct][k]=0.0001;lcp->zeroind[deduct][k]=1;}
						}
	 ...
	 for(t=0;t<TN;t++){
		 for(i=0;i<NSIMUL;i++) 
			 if(healthstate[i][t]!=4){
				 ...
				 if (healindex!=3)
					 constraint=A+ii*LTCI[t][healindex]+wdis[s->curWW[i]]-M[t][healindex];
				 else 
					constraint=A+(ii*LTCI[t][healindex]-M[t][healindex])*(s->tempIndex[i] > deductgrid-2)+ii*LTCI[t][0]*(s->tempIndex[i]<=deductgrid-2)+wdis[s->curWW[i]];
				 ...
				 }else if(para.Mcaid==1 &&  constraint < (Cbar+Wbcar) &&  constraint-wdis[s->curWW[i]]/(1+r)< Cbar && healindex > 1 )
				 {
					 ...
					 if(ii==0)
						 ...
						 s->insurance[i] =s->insurance[i]+ (1/rfactor[t])*LTCI[t][healthstate[i][t]];
						 s->Medicaid[i]=s->Medicaid[i]+ (1 / rfactor[t])*((M[t][healindex]-LTCI[t][healindex])-max((double)0,(double)(wextra+(A+r*wdis[s->curWW[i]]/(1+r) -Cbar2))));
					 ...
					 s->nextWW[i]=Astream[s->tempIndex[i]][t][healthstate[i][t]][s->curWW[i]];
					 if (healthstate[i][t]>=1 && healthstate[i][t]!=3 ) s->insurance[i]=s->insurance[i] + (1/rfactor[t])*LTCI[t][healthstate[i][t]];
					 if(healthstate[i][t]==3 && s->tempIndex[i] > deductgrid-2) s->insurance[i]=s->insurance[i] + (1/rfactor[t])*LTCI[t][healthstate[i][t]];
				 }
				 ...
				 //index for each period in hosptal
					 s->tempIndex[i]=s->tempIndex[i]*(healthstate[i][t]==3);
					 s->tempIndex[i]=s->tempIndex[i]+(healthstate[i][t]==3)*1;
...
void Parallel_LTCI::writeresult(int wealthpercentile ,int gender){
		...
		{  
			...
			out<<endl;
			out<<"simulate"<<endl;
			out<<Digram.MMstar/Digram.Medcost<<endl;
			out<<"implicit tax                                                  column 3 "<<endl; 
			out<<(MUstar - Mstar)/Istarown<<"\t";
			out<<endl;
			out<<"simulate"<<endl;
			out<<(Digram.MUstar-Digram.MMstar)/Digram.IIstar<<endl;

			out<<"Net load														column 4"<<endl; 
			out<<(1-(Istarown-(MUstar-Mstar))/(Istarown/para.MW))<<"\t";
			out<<endl;
			out<<"simulate"<<endl;
			out<<(1-(Digram.IIstar-(Digram.MUstar-Digram.MMstar))/(Digram.IIstar/para.MW))<<endl;
